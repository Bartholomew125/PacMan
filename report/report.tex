...
Dette tillader os at \textit{nedarve} fra den når vi skal lave ting der skal
bevæge sig som \class{Ghost} og \class{PacMan}. Udover \class{Moveable} klassen,
har vi også en klasse, \class{Pos2D}, til at beskrive positioner som ikke skal
kunne bevæge sige. Fra denne klasse kan vi så nedarve klasser som \class{Pill}
og \class{Wall}, da de bare er positioner, men vi godt vil kunne skelne imellem
dem. På denne måde benytter vi \textit{klasseafhængighedsprincippet} til at
simplificerer koden, og gøre det nemmere at udvide med nye features. Alle disse
afhængigheder samt nedarvelser, kan ses på ovenstående billede (se
\autoref{UML-diagram-Model}).
...
ændre sig i de forskellige stadier. På denne måde kan vi udvide denne stadie
når vi skal lave en konkret stadie, så som \class{PowerState}, og så bare
specificere for alle attributterne deres værdier. Så med hjælp fra
\method{setState} metoden i \class{Game} kan vi ændre på stadiet af spillet ved
bare at give spillet et nyt stadie som har supertypen \class{State}.

Til formål at forbedre og forsimple denne interraktion, lavede vi klassen
\class{StateController}. Denne klasse har også en metode \method{setState}, og
kalder \method{setState}. Men hvis det så er en stadie, som \class{PowerState},
der skal køre i en specifik mængde tid, så sørger controlleren også for det,
igennem dens \code{update()} metode.
...
tal, så skal der noget smart logik til for at PacMan kan bevæge sig omkring
hjørner i labyrinten, uden man sidder fast, da den består af en masse væge på
...
For at lave alt dette med retninger, lavede vi en \code{enum} klasse. Der er
fire retninger vi tillader: op, ned, højre og venstre. For at finde ud af
hvilken en af disse retninger vores tastetryk svarer til, lavede vi også en
\class{DirectionAdapter} klasse, som står for at konverterer imellem
...
Vi valgte at implementere forskellige måder for
spøgelserne i PacMan til at jage PacMan(spilleren). Som vores primære metoder
valgte vi to kendte søgealgoritmer, nemlig \textit{Dybde-Først søgning} \&
\textit{Bredde-Først søgning}. Vores grund for at bruge to søgealgoritmer og
ikke kun én, var for at sikre lidt uforudselighed i spillet, da spøgelserne så
skulle tage forskellige ruter mod PacMan.

\textbf{Node klassen}

For at kunne skabe fundamentet for at behandle vores labyrint som et binært træ
skabte vi klassen \class{Node}. \class{Node} klassen består af en forældre
...
\textbf{Grænseflade for Søgealgoritmer}

Grænsefladen \class{SearchAlgorithm} var ment som en måde at kunne give metoder
til vores abstrakte klasse \class{AbstracSearchAlgorithm}. De metoder vi valgte
at give videre var to setters, \method{setGoal} \& \method{setStart}, samt en
metode \method{search} og en getter \method{getMoveStack}. Metoden
\method{getMoveStack} er vores metode for at hente den sti af positioner
spøgelserne skal gå for at nå PacMan's position. Metoden \method{search} ville
blive brugt som en måde at søge efter et mål med den valgte søge strategi. 

\textbf{Abstrakt Søgealgoritme Klasse}

Vores plan med \class{AbstractSearchAlgorithm} som en abstrakt klasse var, så
kunne vi bruge \class{AbstractSearchAlgorithm} som skabelon for andre
søgealgoritmer, og implementere \class{SearchAlgorithm} grænsefladen for at
abstrahere de muligheder, de kommende søgealgoritmer skal have. Vi følte, at det
var en god idé at limitere metoderne tilgængelige for de kommende søgealgoritmer
for at undgå unødvendig komplesiktet og uforudselighed. 
...
højre, venstre) ud fra nuværende position. Ved brug af en \textit{for-løkke}, en
\textit{boolean} metode \method{isWallAt} fra \class{Maze} klassen og to lokale
\textit{Integer} variabler kan vi så sikre, at spøgelsernes nye positioner er
lovlige(der er ikke en væg) og heller ikke uden for labyrinten.

\textbf{Dybde-Først Søgning}

I \textit{Dybde-Først søgning} er strategien at traversere et binært træ ved at
...
fra efterkommer noden op mod roden og udforsker de forskellige efterkommeres
grene for at finde målet der. Hvis målet ikke findes returneres fejl.

For at implementere \textit{Dybde-Først søgning} brugte vi pseudokoden fra
wikipedia(INSERT KILDEHENVISNING HERE) og oversatte den til vores program. For
at kunne bruge metoden \method{getNeighbours} fra abstrakt klassen, skal
\class{DepthFirstSearch} klassen have adgang til vores labyrinten i
\class{DepthFirstSearch} konstruktøren. Ved at følge pseudokoden,
\method{getNeighbours} og to metoder fra \class{Node} klassen undersøgte vi
stier af positioner til PacMan, og returnerede en stack af bevægelser som kunne
gives til \class{GhostController} og opdatere spøgelserne med \method{update}
metoden fra \class{GhostController}. 

En anden ændring er ved \code{ArrayList} medfødte metode \method{contains}. Når
man i en \code{ArrayList} vil undersøge om den indeholder et objekt, kaldes
\method{contains} der bruges som en boolean til at sammenligne et objekt med
objekterne i listen. Den metode har vi måtte overskride for at passe til
\class{Pos2D} objektet. Ændringen er en \textit{for-løkke} der sammenligner
\class{Pos2D}'ers X og Y værdier.

\textbf{Breadth-FirstSearch klasse}

I \textit{Bredde-Først søgning} er strategien at traversere et binært træ ved at
søge først venstre gren og så hver gren i et binært træ (i samme dybde). Vi går
derefter først dybere i træet når hver gren er undersøgt. Hvis målet ikke findes
returneres fejl.

For at implementere \textit{Bredde-Først søgning} brugte vi pseudokoden fra
wikipedia(INSERT KILDEHENVSNING HERE) og oversatte den til vores program. For at
kunne bruge metoden \method{getNeighbours} fra abstrakt klassen, skal
\class{BreadthFirstSearch} klassen have adgang til vores labyrint i
\class{BreadthFirstSearch} konstruktøren. Ved at følge pseudokoden,
\method{getNeighbours} og to metoder fra \class{Node} klassen undersøgte vi
stier af positioner til PacMan, og returnerede en stack af bevægelser som kunne
gives til \class{GhostController} og opdatere spøgelserne med \method{update}
metoden fra \class{GhostController}. Da vi brugte lænkede lister som vores kø i
\class{BreadthFirstSearch}, og lænkede lister i Java også har metoden
\method{contains}, valgte vi at overskride \method{contains} og ændre den for
samme grunde og på samme vis som i \class{DepthFirstSearch} klassen.
...
én frame varer i nanosekunder. På denne måde kan vi i \method{getFrame} metode
...
finder vi hvor langt inde i den animation cyklus vi er ved at tage den nuværende
tid modulus længden af animationen. Med dette kan vi så dividerer med længden af
én frame og få det frame indeks som vi er nået til.
...
har lavede vi en klasse \class{TextualMaze}. Dens formål er kun at læse
